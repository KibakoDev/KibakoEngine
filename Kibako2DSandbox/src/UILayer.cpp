...
void UILayer::OnAttach()
{
    ...
    m_hierarchyList = m_mainMenuDoc->GetElementById("hierarchy_list");
    if (!m_hierarchyList) {
    ...
    if (auto* apply = m_mainMenuDoc->GetElementById("btn_apply")) {
...
void UILayer::RefreshInspectorFromSelection()
{
    ...
    auto* hint = m_mainMenuDoc->GetElementById("inspector_hint");
    ...
    if (m_selectedEntityId == 0 || !m_scene.FindEntity(m_selectedEntityId)) {
        ...
        SetInputValue(m_mainMenuDoc, "ins_name", "");
        SetInputValue(m_mainMenuDoc, "ins_pos_x", "0");
        SetInputValue(m_mainMenuDoc, "ins_pos_y", "0");
        SetInputValue(m_mainMenuDoc, "ins_rot", "0");
        SetInputValue(m_mainMenuDoc, "ins_scale_x", "1");
        SetInputValue(m_mainMenuDoc, "ins_scale_y", "1");
        return;
    }
    ...
    SetInputValue(m_mainMenuDoc, "ins_name", name);
    SetInputValue(m_mainMenuDoc, "ins_pos_x", std::to_string(t.position.x));
    SetInputValue(m_mainMenuDoc, "ins_pos_y", std::to_string(t.position.y));
    SetInputValue(m_mainMenuDoc, "ins_rot", std::to_string(t.rotation));
    SetInputValue(m_mainMenuDoc, "ins_scale_x", std::to_string(t.scale.x));
    SetInputValue(m_mainMenuDoc, "ins_scale_y", std::to_string(t.scale.y));
}

void UILayer::ApplyInspectorToSelection()
{
    if (!m_mainMenuDoc || m_selectedEntityId == 0)
        return;
    ...
    // Read fields
    const std::string name = GetInputValue(m_mainMenuDoc, "ins_name");

    const float px = ParseFloat(GetInputValue(m_mainMenuDoc, "ins_pos_x"), e->transform.position.x);
    const float py = ParseFloat(GetInputValue(m_mainMenuDoc, "ins_pos_y"), e->transform.position.y);
    const float rot = ParseFloat(GetInputValue(m_mainMenuDoc, "ins_rot"), e->transform.rotation);
    const float sx = ParseFloat(GetInputValue(m_mainMenuDoc, "ins_scale_x"), e->transform.scale.x);
    const float sy = ParseFloat(GetInputValue(m_mainMenuDoc, "ins_scale_y"), e->transform.scale.y);
...
{
    if (!m_hierarchyList || !m_mainMenuDoc)
        return;
    ...
    for (const auto& e : m_scene.Entities()) {
        ...
        Rml::ElementPtr btn = m_mainMenuDoc->CreateElement("button");
        if (!btn)
            continue;
